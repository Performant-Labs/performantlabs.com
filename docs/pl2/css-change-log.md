...
[Layer 5] .dy-section--proof-strip .proof-item / .proof-item__n / .proof-item__label / .proof-item__sub / .pill  css/components/dy-section.css  2026-07-22 (Wave 2 sub-phase B, #281)
  Ruling: new product social-proof strip (proof-item value/label markup, dripyard_base:pill MIT badge). L1 not config. L3 too broad — would re-style every statistic/pill instance in cream-zone sections site-wide. L5 correct, scoped to .dy-section--proof-strip. Compact mono figure #8E4A2A on cream #F5EFE2 = 5.79:1 AA/AAA; muted labels #5C544C on #F5EFE2 = 6.48:1 AAA; MIT pill text/border #8E4A2A on white = 6.64:1 AA/AAA. Component-reuse correction: dripyard_base:statistic (wireframe's suggested mapping) was tried first but its countUp.js behavior strips non-digit characters and animates from 0, corrupting non-numeric placeholder values and the version string live — switched to dripyard_base:text (no JS behavior), same pattern already used elsewhere on this page.

[Layer 5] Nav visual weight — .site-header .primary-menu__link--level-1[href="..."] font-weight/color split (weighted: /aftersight, /open-source-projects, /articles; demoted: /services, /how-we-do-it, /about-us, /contact-us), desktop (>1000px) + mobile (<=1000px) variants  css/components/header.css  2026-07-22 (Wave 2 sub-phase B, #281)
  Ruling: L1 not config (no menu-link weight field exists). L3 not a token concern. L5 correct, href-keyed selectors matching this file's existing pattern. Desktop weighted #2A2520/demoted #5C544C on white = 15.17:1/7.43:1 AAA. Mobile nav panel is itself .theme--black (dark overlay, confirmed live) — weighted #FFFFFF/demoted #B8AFA0 on #0E1014 = 19.04:1/8.77:1 AAA, both resolving correctly through the context-sensitive --theme-text-color-loud/-medium tokens once specificity was bumped to (0,3,0) via the .site-header ancestor (initial (0,2,0) attempt tied with and lost to neonbyte's own primary-menu-narrow.css rule — found live during T2 self-verification, fixed before handoff).

[Layer 5] .dy-section--feature-anatomy-reverse .grid-cell--feature-copy/-crop order; .grid-wrapper--feature-anatomy row-gap  css/components/grid-wrapper.css  2026-07-22 (Wave 2 sub-phase C, #282)
  Ruling: alternating 2-col feature-anatomy layout (History/Failure analytics/Search). Base 2-up split handled entirely by grid-cell's own --grid-cell-columns-* custom properties (no new CSS needed for that half). Visual-only reversal for feature 2 via `order` (DOM order stays copy-then-crop for screen readers/keyboard — WCAG 1.3.2 compliant). L1 not config. L3 not a token. L5 correct, marker-class scoped, matching this file's --2col/--3col precedent.

[Content, not CSS] Services section (uuid a8933523) + logo-grid section (uuid d239b0c9) `theme` prop: white -> light (cream)  canvas_page 20 via scripts/sprint-wave2-282-features-services-relocate.php  2026-07-22
  Ruling: approved wireframe demotes services + client logos to the cream register (position + tonal-shift demotion mechanism, RATIONALE.md). Content (verbatim #269 header, 3 service cards, 6 logo items) untouched — only the section's own `theme` Canvas input changes. Section relocated below the 3 new feature-anatomy sections (was directly after hero) via the assembly script's insertion-index logic; component_version untouched on every descendant.

[Content, not CSS] New visually-hidden H2 "Product features" (dripyard_base:heading, modifier_classes: visually-hidden) as feature-1 section header  canvas_page 20 via scripts/sprint-wave2-282-features-services-relocate.php  2026-07-22
  Ruling: without a section header, the 3 new feature-anatomy sections produced a direct H1 -> H3 heading-hierarchy skip (found live during T2 self-verification). Added a single visually-hidden H2 group label (Drupal-core .visually-hidden utility class, same pattern the nav's own menu-block title already uses sitewide) before the first feature section only, restoring a clean H1 -> H2 -> H3 sequence for the group of three. No visual change (1x1px clipped, confirmed live via getComputedStyle).

[Layer 5] Nav visual weight REMOVED — deleted the #276/#281 href-keyed font-weight/font-size/opacity/color split on .site-header .primary-menu__link--level-1[href="..."] (both desktop >1000px and mobile <=1000px @media blocks)  css/components/header.css  2026-07-26 (fix/uniform-nav-weight, uniform nav)
  Ruling: product decision — every top-level main-menu link must render IDENTICALLY (one uniform weight/size/opacity/color). This is a pure REMOVAL of the #281 differentiation, not a new override: with both weighted/demoted rule pairs deleted, all level-1 links fall back to this file's existing uniform baseline — desktop Poppins 15px, --theme-text-color-primary, font-weight 500 (via --top-level-link-font-weight token) at .site-header .primary-menu__link--level-1[class] (0,3,0); mobile Poppins font-weight 500 at the narrow-nav rule. Menu ORDER + LABELS are #270 config scope and untouched (no menu/config change; CSS-only). WCAG unaffected — uniform links use --theme-text-color-primary #2A2520 on white = 15.17:1 AAA (desktop) and resolve through the .theme--black mobile panel token unchanged.
